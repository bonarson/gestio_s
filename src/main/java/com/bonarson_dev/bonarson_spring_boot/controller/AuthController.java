...
import com.bonarson_dev.bonarson_spring_boot.service.ClientService;
...
public class AuthController {
    ...
    public AuthController(ClientService clientService, DataSource dataSource) {
    ...
    public ResponseEntity<String> login(@RequestBody LoginForm loginForm) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            if (clientService.authenticateClient(email, password, connection)) {
                return ResponseEntity.ok("Information correcte");
            } else {
    ...
    public ResponseEntity<String> changePassword(@RequestBody ChangePasswordForm changePasswordForm) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            if (clientService.changePassword(email, currentPassword, newPassword, connection)) {
